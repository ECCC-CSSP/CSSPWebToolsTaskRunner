...
{
    public class DocxServiceRoot
    {
        #region Variables
        #endregion Variables

        #region Properties
        public GeneratedClassRootDocx _Docx { get; set; }
        public TaskRunnerBaseService _TaskRunnerBaseService { get; private set; }
        #endregion Properties

        #region Constructors
        public DocxServiceRoot(TaskRunnerBaseService taskRunnerBaseService)
        {
            _TaskRunnerBaseService = taskRunnerBaseService;
            _Docx = new GeneratedClassRootDocx(_TaskRunnerBaseService);
        }
    ...
    public class GeneratedClassRootDocx
    {
        #region Properties
        public TaskRunnerBaseService _TaskRunnerBaseService { get; private set; }
        ...
        #region Constructors
        public GeneratedClassRootDocx(TaskRunnerBaseService taskRunnerBaseService)
        {
        ...
        {
            DocxBase.CurrentTitle = DocxServiceRootRes.Title;
        ...
        private void GenerateMainDocumentPart1Content(MainDocumentPart mainDocumentPart1)
        {
            ...
            GenerateRootDocument(document);

            mainDocumentPart1.Document = document;
        }
        private void GenerateRootDocument(Document document)
        {
            Body body = new Body();
            ...
            if (!string.IsNullOrWhiteSpace(tvItemModelRoot.Error))
            {
                ...
                DocxBase.AddRunWithCurrentParagraphStyle(paragraph, tvItemModelRoot.Error);
            }
            ...
            URL = _TaskRunnerBaseService.GetUrlFromTVItem(tvItemModelRoot);
            run = DocxBase.AddRunHyperlink(paragraph, URL, tvItemModelRoot.TVText);

            List<TVItemModel> tvItemModelCountryList = tvItemService.GetChildrenTVItemModelListWithTVItemIDAndTVTypeDB(tvItemModelRoot.TVItemID, TVTypeEnum.Country);

            foreach (TVItemModel tvItemModel in tvItemModelCountryList)
            {
                //tvItemStatService.SetTVItemStatForTVItemIDAndParentsTVItemID(tvItemModel.TVItemID);
            }

            foreach (TVItemModel tvItemModelCountry in tvItemModelCountryList)
            {
                DocxBase.CurrentParagraphStyle = ParagraphStyleEnum.Heading2;
                DocxBase.CurrentJustificationValue = JustificationValues.Center;
                paragraph = DocxBase.AddParagraph(body);

                URL = _TaskRunnerBaseService.GetUrlFromTVItem(tvItemModelCountry);
                run = DocxBase.AddRunHyperlink(paragraph, URL, tvItemModelCountry.TVText);
                ...
                List<TVItemStatModel> tvItemStatModelList = tvItemStatService.GetTVItemStatModelListWithTVItemIDDB(tvItemModelCountry.TVItemID);
                ...
                foreach (TVItemStatModel tvItemStatModel in tvItemStatModelList)
                {
                    ...
                    //if (tvItemStatModel.ChildCount > 10000)
                    //{
                    //    DocxBase.CurrentFontColor = System.Drawing.Color.Green;
                    //    DocxBase.CurrentFontSize = 22;
                    //    run = DocxBase.AddRunWithCurrentFontStyle(paragraph, tvItemStatModel.ChildCount.ToString());
                    //}
                    //else
                    //{
                ...
                DocxBase.SetDefaults();
                DocxBase.CurrentParagraphStyle = ParagraphStyleEnum.Normal;
                paragraph = DocxBase.AddParagraph(body);
                DocxBase.AddRunWithCurrentParagraphStyle(paragraph, "Normal ");

                DocxBase.CurrentRunStyle = RunStyleEnum.Strong;
                DocxBase.AddRunWithCurrentParagraphStyle(paragraph, "Strong");
                DocxBase.AddParagraph(body);
            }
