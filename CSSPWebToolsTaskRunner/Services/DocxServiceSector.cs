...
{
    public class DocxServiceSector
    {
        #region Variables
        #endregion Variables

        #region Properties
        public GeneratedClassSectorDocx _Docx { get; set; }
        public TaskRunnerBaseService _TaskRunnerBaseService { get; private set; }
        #endregion Properties

        #region Constructors
        public DocxServiceSector(TaskRunnerBaseService taskRunnerBaseService)
        {
            _TaskRunnerBaseService = taskRunnerBaseService;
            _Docx = new GeneratedClassSectorDocx(_TaskRunnerBaseService);
        }
    ...
    public class GeneratedClassSectorDocx
    {
        #region Properties
        public TaskRunnerBaseService _TaskRunnerBaseService { get; private set; }
        ...
        #region Constructors
        public GeneratedClassSectorDocx(TaskRunnerBaseService taskRunnerBaseService)
        {
        ...
        {
            DocxBase.CurrentTitle = DocxServiceSectorRes.Title;
        ...
        private void GenerateMainDocumentPart1Content(MainDocumentPart mainDocumentPart1)
        {
            ...
            GenerateSectorDocument(document);

            mainDocumentPart1.Document = document;
        }
        private void GenerateSectorDocument(Document document)
        {
            Body body = new Body();
            ...
            if (!string.IsNullOrWhiteSpace(tvItemModelSector.Error))
            {
                ...
                DocxBase.AddRunWithCurrentParagraphStyle(paragraph, tvItemModelSector.Error);
            }
            ...
            foreach (TVItemModel tvItemModelSubsector in tvItemModelSubsectorList)
            {
                ...
                URL = _TaskRunnerBaseService.GetUrlFromTVItem(tvItemModelSubsector);
                run = DocxBase.AddRunHyperlink(paragraph, URL, tvItemModelSubsector.TVText);
