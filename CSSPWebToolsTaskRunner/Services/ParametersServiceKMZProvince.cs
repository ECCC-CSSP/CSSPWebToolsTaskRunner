...
namespace CSSPWebToolsTaskRunner.Services
{
    ...
    {
        private bool GenerateKMZProvince(FileInfo fi, StringBuilder sbKMZ, string parameters, ReportTypeModel reportTypeModel)
        {
            switch (reportTypeModel.UniqueCode)
            {
                case "ProvinceTestFRKMZ":
                case "ProvinceTestENKMZ":
                    {
                        if (!GenerateKMZProvince_ProvinceTestKMZ(fi, sbKMZ, parameters, reportTypeModel))
                        {
                            return false;
                        }
                    }
        ...
        private bool GenerateKMZProvince_ProvinceTestKMZ(FileInfo fi, StringBuilder sbKMZ, string parameters, ReportTypeModel reportTypeModel)
        {
        ...
        private bool GenerateKMZProvince_NotImplementedKMZ(FileInfo fi, StringBuilder sbKMZ, string parameters, ReportTypeModel reportTypeModel)
        {
            sbKMZ.AppendLine(@"<?xml version=""1.0"" encoding=""UTF-8""?>");
            sbKMZ.AppendLine(@"<kml xmlns=""http://www.opengis.net/kml/2.2"" xmlns:gx=""http://www.google.com/kml/ext/2.2"" xmlns:kml=""http://www.opengis.net/kml/2.2"" xmlns:atom=""http://www.w3.org/2005/Atom"">");
            sbKMZ.AppendLine(@"<Document>");
