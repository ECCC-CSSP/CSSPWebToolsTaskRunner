...
namespace CSSPWebToolsTaskRunner.Services
{
    public class KmzServiceRoot
    {
        ...
        #region Constructors
        public KmzServiceRoot(TaskRunnerBaseService taskRunnerBaseService)
        {
        ...
        public void Generate(FileInfo fi)
        {
            ...
            List<MapInfoPointModel> mapInfoPointModelList = mapInfoService._MapInfoPointService.GetMapInfoPointModelListWithTVItemIDAndTVTypeAndMapInfoDrawTypeDB(tvItemModelRoot.TVItemID, TVTypeEnum.Root, MapInfoDrawTypeEnum.Point);

            sbKMZ.AppendLine(@"	<Folder>");
            sbKMZ.AppendLine(@"	<name>Root</name>");

            // Doing Point
            sbKMZ.AppendLine(@"	<Placemark>");
            sbKMZ.AppendLine(@"	<name>" + tvItemModelRoot.TVText + "</name>");
		...
		//sbKMZ.AppendLine(@"	<description>");
            ...
            // Doing Polygon
            sbKMZ.AppendLine(@"	<Placemark>");
            sbKMZ.AppendLine(@"		<name>" + tvItemModelRoot.TVText + " (poly)</name>");
            sbKMZ.AppendLine(@"<styleUrl>#msn_ylw-pushpin</styleUrl>");
            //sbKMZ.AppendLine(@"	<description>");
            //sbKMZ.AppendLine(@"<![CDATA[");
            //sbKMZ.AppendLine(@"<a href=""" + _TaskRunnerBaseService.GetUrlFromTVItem(tvItemModelRoot) + @""">" + tvItemModelRoot.TVText + "</a>");
            ...
            sbKMZ.AppendLine(@"		" + (mapInfoPointModelList[0].Lng - 1) + "," + (mapInfoPointModelList[0].Lat - 1) + ",0");
            sbKMZ.AppendLine(@"		" + (mapInfoPointModelList[0].Lng + 1) + "," + (mapInfoPointModelList[0].Lat - 1) + ",0");
            sbKMZ.AppendLine(@"		" + (mapInfoPointModelList[0].Lng + 1) + "," + (mapInfoPointModelList[0].Lat + 1) + ",0");
            sbKMZ.AppendLine(@"		" + (mapInfoPointModelList[0].Lng - 1) + "," + (mapInfoPointModelList[0].Lat + 1) + ",0");
            sbKMZ.AppendLine(@"		" + (mapInfoPointModelList[0].Lng - 1) + "," + (mapInfoPointModelList[0].Lat - 1) + ",0");
            ...
            sbKMZ.AppendLine(@" <Folder>");
            sbKMZ.AppendLine(@"	<name>Countries</name>");
            ...
            foreach (TVItemModel tvItemModelCountry in tvItemModelCountryList)
            {
                // Doing point
                sbKMZ.AppendLine(@"	<Placemark>");
                sbKMZ.AppendLine(@"	<name>" + tvItemModelCountry.TVText + "</name>");
                sbKMZ.AppendLine(@"<styleUrl>#msn_ylw-pushpin</styleUrl>");
                //sbKMZ.AppendLine(@"	<description>");
                //sbKMZ.AppendLine(@"<![CDATA[");
                ...
                mapInfoPointModelList = mapInfoService._MapInfoPointService.GetMapInfoPointModelListWithTVItemIDAndTVTypeAndMapInfoDrawTypeDB(tvItemModelCountry.TVItemID, TVTypeEnum.Country, MapInfoDrawTypeEnum.Point);
                ...
                // Doing Polygon
                sbKMZ.AppendLine(@"	<Placemark>");
                sbKMZ.AppendLine(@"		<name>" + tvItemModelCountry.TVText + " (poly)</name>");
                sbKMZ.AppendLine(@"<styleUrl>#msn_ylw-pushpin</styleUrl>");
                //sbKMZ.AppendLine(@"	<description>");
                //sbKMZ.AppendLine(@"<![CDATA[");
                //sbKMZ.AppendLine(@"<a href=""" + _TaskRunnerBaseService.GetUrlFromTVItem(tvItemModelCountry) + @""">" + tvItemModelCountry.TVText + "</a>");
