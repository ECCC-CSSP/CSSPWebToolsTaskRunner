...
namespace CSSPWebToolsTaskRunner.Services
{
    ...
    {
        private bool GenerateHTMLSUBSECTOR_FC_SUMMARY_STAT_ALL(StringBuilder sbTemp)
        {
            int Percent = 10;
            string NotUsed = "";
            ...
            _TaskRunnerBaseService.SendStatusTextToDB(_TaskRunnerBaseService.GetTextLanguageFormat1List("Creating_", ReportGenerateObjectsKeywordEnum.SUBSECTOR_FC_SUMMARY_STAT_ALL.ToString()));
            ...
            List<MWQMAnalysisReportParameterModel> mwqmAnalysisReportParameterModelList =
                ...
                && c.AnalysisCalculationType == AnalysisCalculationTypeEnum.AllAllAll).ToList();
            ...
            foreach (MWQMAnalysisReportParameterModel mwqmAnalysisReportParameterModel in mwqmAnalysisReportParameterModelList)
            {
                ...
                {
                    sbTemp.AppendLine($@"<h4>{ TaskRunnerServiceRes.All } { TaskRunnerServiceRes.NotShown }</h4>");
                    continue;
                }
                ...
                sbTemp.AppendLine($@"|||TableCaption|Table 6.2.0a: { TaskRunnerServiceRes.SummaryStatisticsOfFCDensities } ({ TaskRunnerServiceRes.MPN }/100 mL) ({ AllWetDryText } { RunText })|||");
                ...
                foreach (MWQMSiteAnalysisModel mwqmSiteAnalysisModel in mwqmSiteAnalysisModelList)
                {
                    ...
                    if (mwqmSampleAnalysisForSiteModelToUseList.Count >= 4)
                    {
                        ...
                        if (MinYear != null)
                        {
                            if (StatStartYear > MinYear || StatStartYear == 0)
                            {
        ...
        public bool PublicGenerateHTMLSUBSECTOR_FC_SUMMARY_STAT_ALL(StringBuilder sbTemp)
        {
            bool retBool = GenerateHTMLSUBSECTOR_FC_SUMMARY_STAT_ALL(sbTemp);
